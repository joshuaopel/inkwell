...
// A single attempt. Resumes from whatever is already in the .part file.
async function downloadOnce(url, dest, { onProgress, signal, expected = 0 } = {}) {
  ...
  // A download that goes quiet is worse than one that fails: it hangs until
  // some other timeout notices. Watch the byte counter and give up on silence
  // so the retry above can start again from the same place.
  ...
  const stalled = setInterval(() => {
    if (Date.now() - lastData > STALL_MS) ac.abort(new Error('The connection went quiet.'));
  }, 5000);
  ...
  try {
    ...
    if (!res.ok) {
      const err = new Error(`Download failed (HTTP ${res.status}) for ${url.split('/').pop()}`);
...
// Ten gigabytes over a domestic connection will be interrupted. Treating that
// as a failure — which is what the first version of this did, and why the
// biggest model was the one that never finished — throws away everything
// already downloaded. Retry from the part file instead.
async function download(url, dest, opts = {}) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      // The author cancelling is not a network blip.
      if (opts.signal?.aborted) throw e;
      // A 404 or a 403 will say the same thing however many times we ask.
      if (e.status && e.status !== 408 && e.status !== 429 && e.status < 500) throw e;
      lastErr = e;
      if (i === attempts) break;
      const wait = Math.min(30_000, 2000 * 2 ** (i - 1));
      opts.onProgress?.({
        got: await fileSize(dest + '.part'),
        total: opts.expected,
        note: `Connection dropped — picking up where it left off (try ${i + 1} of ${attempts})…`,
      });
      await new Promise((r) => setTimeout(r, wait));
    }
  }
  throw new Error(`${lastErr.message} — gave up after ${attempts} tries. Press the button again to resume.`);
}
...
export async function install({ tier = 'storybook', accel, onProgress = () => {}, signal } = {}) {
  ...
  // Switching models while the old one is loaded would leave the running
  // process serving the previous model for the rest of the session — the new
  // one would download, report success, and change nothing.
  const before = await readState();
  if (before.tier && before.tier !== t.id) await stop();
...
let lastLog = [];
let exitInfo = null;
// What the engine is doing right this second, parsed from its own output, so a
// five-minute drawing shows a moving bar instead of looking like a hang.
let activity = null;

// sd-server reports both phases on the same stream, distinguished by units:
//   |####    | 63/140 - 11.9MB/s     loading weights
//   |####    | 12/26  - 1.23s/it     actually drawing
const TICK = /(\d+)\/(\d+)\s*-\s*([\d.]+)(MB\/s|s\/it|it\/s)/g;
...
function absorb(buf) {
  ...
  // A chunk can carry many carriage-returned updates; only the last one counts.
  let m, last = null;
  while ((m = TICK.exec(text)) !== null) last = m;
  TICK.lastIndex = 0;
  if (!last) return;

  const [, done, total, rate, unit] = last;
  const now = Date.now();
  activity = {
    phase: unit === 'MB/s' ? 'loading' : 'drawing',
    done: +done,
    total: +total,
    rate: `${rate}${unit}`,
    at: now,
    // How long the last step took. On a machine without a graphics card that is
    // minutes, not seconds, and a fixed staleness window would blank the
    // progress out between every tick — on exactly the machines that need it.
    gap: activity && activity.phase !== 'loading' ? now - activity.at : 0,
  };
}

// Progress is stale when the next tick is overdue, not on a fixed clock.
export function engineActivity() {
  if (!activity) return null;
  const allow = Math.max(20_000, (activity.gap || 0) * 3);
  return Date.now() - activity.at > allow ? null : activity;
}
...
export async function start({ onProgress } = {}) {
  ...
  starting = (async () => {
    ...
    child.on('exit', (code) => {
      exitInfo = stopping ? null : { code, log: lastLog.slice(-8).join('') };
      if (!stopping && code) console.error('[engine] sd-server exited', code, exitInfo.log);
      child = null;
      activity = null;
    });

    // Several gigabytes off a slow disk takes as long as it takes. Wait a long
    // while, but say what's happening rather than sitting mute.
    const began = Date.now();
    const deadline = began + 25 * 60_000;
    while (Date.now() < deadline) {
      if (!child) {
        throw new Error(`The drawing engine stopped while starting up.\n\n${exitInfo?.log || lastLog.slice(-6).join('')}`.trim());
      }
      if (await alive(1500)) {
        onProgress?.({ stage: 'ready', note: 'Loaded.' });
        return { ok: true, base: BASE };
      }
      onProgress?.({ stage: 'loading', note: `Loading the ${t.label} model… ${Math.round((Date.now() - began) / 1000)}s` });
      await new Promise((r) => setTimeout(r, 1000));
    }
    const log = lastLog.slice(-6).join('');
    await stop();
    throw new Error(`The drawing engine hasn't finished loading after 25 minutes, so something is wrong.\n\n${log}`.trim());
