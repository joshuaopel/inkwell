...
import { normalizeDesign, normalizeArt, DEFAULT_DESIGN, presetDesign } from './design.js';
...
export async function listBooks() {
  ...
  for (const e of entries) {
    ...
    out.push({
      id: meta.id, title: meta.title, author: meta.author, kind: meta.kind || 'novel',
...
export async function createBook({ title, author, premise, genre, pov, tense, wordTarget, kind, ageBand }) {
  ...
  const meta = {
    ...
    // What kind of book this is decides its structure, its interview and its
    // page design. See lib/kinds.js.
    kind: kind === 'picture' ? 'picture' : 'novel',
    ageBand: ageBand || 'picture',
  ...
  await writeJson(path.join(bookDir(bookId), 'bible.json'), {
    ...
    // Illustrated books: one locked style line for every picture, plus the
    // palette. Per-character looks hang off each character entry.
    visualStyle: '', palette: [], refrain: '',
  });
  ...
  // A picture book that opens as a 6×9 justified novel page is wrong before
  // you've typed a word, so give it the square illustrated design up front.
  // Novels keep having no design.json until you change something.
  const preset = KINDS[meta.kind]?.designPreset;
  if (preset && preset !== 'classic') {
    await writeJson(path.join(bookDir(bookId), 'design.json'), presetDesign(preset));
  }
  return meta;
}
...
export async function saveOutline(bookId, outline) {
  ...
  const chapters = (outline.chapters || []).map((c) => ({
    ...
    // Illustrated books: what this spread's picture shows, and the prompt last
    // used to render it, so it can be regenerated or nudged.
    art: typeof c.art === 'string' ? c.art : '',
    artPrompt: typeof c.artPrompt === 'string' ? c.artPrompt : '',
...
export const DEFAULT_SETTINGS = {
  ...
  // Local image generation for illustrated books. Off until you point it at a
  // running ComfyUI or AUTOMATIC1111.
  // Drawing is off until you ask for it. When you do, the default is the engine
  // Inkwell installs itself rather than something you have to go and set up.
  image: { enabled: false, backend: 'bundled', baseUrl: '', model: '', size: 'square', steps: 0, negative: '', workflow: '' },
...
export async function getSettings() {
  ...
  return {
    ...
    image: { ...DEFAULT_SETTINGS.image, ...(s.image || {}) },
...
export async function saveSettings(patch) {
  ...
  const next = {
    ...
    image: { ...DEFAULT_SETTINGS.image, ...(patch.image || {}) },
    cloud,
  };
